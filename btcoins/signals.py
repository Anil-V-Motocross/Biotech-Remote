...
@receiver(post_save, sender=OrderStatus)
def credit_coins_after_delivery(sender, instance, created, **kwargs):
    """Credits coins only if all required statuses exist before marking order as DELIVERED or PICKUP_COMPLETED"""
    ...
    print(f"Processing Order: {order.order_id} | Delivery Option: {delivery_option}")

    # 🚚 **For Home Delivery**
    if delivery_option != 'PickUpStore' and instance.status == 'DELIVERED':  
        required_statuses = {'INITIATED', 'PROCESSING', 'ORDER_CONFIRMED', 'DISPATCHED', 'ON_THE_WAY', 'OUT_FOR_DELIVERY'}

    # 🏬 **For Pickup Orders**
    elif delivery_option == 'PickUpStore' and instance.status == 'PICKUP_COMPLETED':
        required_statuses = {'INITIATED', 'PROCESSING', 'ORDER_CONFIRMED', 'READY_FOR_PICKUP', 'PICKUP_COMPLETED'}

    else:
        return  # Ignore other statuses

    # Check if order was already marked as completed before
    if OrderStatus.objects.filter(order=order, status=instance.status).exclude(id=instance.id).exists():
        print(f"Order is already marked as {instance.status}. No coins will be credited.")
        return
