...
#         return Response({'message': 'Invalid order source.'}, status=status.HTTP_400_BAD_REQUEST)
...
#                 return Response({'message': f'Product {item.product_id.name} is out of stock.'}, status=status.HTTP_400_BAD_REQUEST)
...
#             else:
#                 return Response({'message': 'Invalid or ineligible coupon.'}, status=status.HTTP_400_BAD_REQUEST)
...
#         "is_shop_the_look": is_shop_the_look  # New field to store whether it's "Shop The Look"
#     }
...
#         return Response({'message': 'Error', 'errors': order_serializer.errors}, status=status.HTTP_400_BAD_REQUEST)

#  # ✅ Save Order Items
#     order_item_instances = []
#     print("order instance  --:", order_instance)
#     for item in order_items:
#         item['order'] = order_instance  # ✅ Assign order instance to each order item
...
#             return Response({'message': 'Error', 'errors': order_item_serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
...
def place_order(request):
    ...
    # ✅ Save Order Items
    order_item_instances = []
    print("Order instance ID:", order_instance.id)  # Debugging: Print the order instance ID
