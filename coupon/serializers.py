...
def validate(self, data):
    ...
    applicable_categories = data.get('applicable_categories', None)
    applicable_products = data.get('applicable_products', None)
    is_first_order = data.get('is_first_order', None)

    # Apply validation only during creation (POST)
    if request_method == 'POST':
        # Get instance values if available
        applicable_categories = applicable_categories or self.instance.applicable_categories.all() if self.instance else []
        applicable_products = applicable_products or self.instance.applicable_products.all() if self.instance else []
        is_first_order = is_first_order if is_first_order is not None else (self.instance.is_first_order if self.instance else False)

        selected_fields = sum(bool(x) for x in [applicable_categories, applicable_products, is_first_order])
        if selected_fields != 1:
            raise serializers.ValidationError("A coupon must apply to either categories, products, or be a first-order coupon, but not multiple.")

    # Apply validation in PATCH only if these fields are being updated
    elif request_method == 'PATCH':
        if any(field in data for field in ['applicable_categories', 'applicable_products', 'is_first_order']):
            selected_fields = sum(bool(x) for x in [applicable_categories, applicable_products, is_first_order])
            if selected_fields != 1:
                raise serializers.ValidationError("A coupon must apply to either categories, products, or be a first-order coupon, but not multiple.")
